...
- 활동 상세 진입 시 최신 원본 이벤트의 15초 전부터 55초 후까지 녹화를 한 번 요청하고, 준비가 끝나면 영상 자체의 native controls로 재생한다. 별도 `녹화 영상 재생` 버튼은 두지 않는다.
...
- 녹화 패널은 `영상 준비 중`, `재생 가능`, `녹화 없음`, `불러오기 실패`를 텍스트로 구분한다. `switch.main_camera_recordings`와 Frigate clip 개수는 과거 녹화의 존재 여부 판정에 사용하지 않는다.
- 녹화 패널 아래에는 이벤트 레일과 영상 범위를 물리적으로 분리한 하나의 24시간 타임라인을 둔다. 이벤트 레일은 5분 활동 구간을 유지하되 화면 폭에서 충돌하는 구간을 군집화하고, 각 군집의 포인터 영역은 최소 `44×44px`이다.
- 모든 이벤트의 `15초 전–55초 후` 범위는 요청 후보로만 표시하고, 실제 HLS manifest가 준비된 현재 선택 범위만 채워진 녹화색으로 확정한다. 조회하지 않은 후보를 녹화 완료로 표시하지 않는다.
