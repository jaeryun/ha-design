...
- 선택 날짜 아래 24시간 바는 `00·04·08·12·16·20·24` 4시간 눈금을 사용하며, 구간의 실제 시작 위치와 지속시간을 1440분 비율로 표시한다. 단일 시점은 폭이 없는 점 마커로 표시한다.
...
- 실시간 여부는 영상 타임스탬프로 판단하므로 `LIVE` 라벨과 파란 점을 표시하지 않는다.
